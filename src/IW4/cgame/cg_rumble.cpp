...
static int ShouldNotRumble(int localClientNum)
{
...
static struct ActiveRumble* GetDuplicateRumbleIfExists(const struct cg_s* cgameGlob, struct ActiveRumble* arArray, const struct RumbleInfo* info, bool loop, enum RumbleSourceType type, int entityNum, const float* pos)
{
...
static void PlayRumbleInternal(int localClientNum, const char* rumbleName, bool loop, enum RumbleSourceType type, int entityNum, const float* pos, float scale, bool updateDuplicates)
{
...
void CG_PlayRumbleOnClientScaledWithUpdate(int localClientNum, const char* rumbleName, float scale)
{
