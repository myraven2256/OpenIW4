...
static const char* RawBufferOpen(const char* name, const char* formatFullPath, char* buf, int size)
{
...
static bool ApplyTokenToField(unsigned int fieldNum, const char* token, struct visionSetVars_t* settings)
{
...
static void LerpVisionSet(const struct visionSetVars_t* from, const struct visionSetVars_t* to, struct visionSetVars_t* result, enum visionSetLerpStyle_t style, float fraction)
{
...
static void FadeRefDef(struct refdef_t* rd, float brightness)
{
