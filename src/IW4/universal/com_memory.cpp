...
static int CanUseServerLargeLocal()
{
...
void* Hunk_AllocDebugMemAligned(int size, int alignment)
{
...
struct HunkUser* Hunk_UserCreate(int maxSize, const char* name, bool fixed, int type)
{
...
unsigned int Hunk_UserFreeMemoryAmount(struct HunkUser* user)
{
...
char* Hunk_CopyNString(struct HunkUser* user, const char* in, int len)
{
