...
float Vec3DistanceSqToSegment(const float* pt, const float* segStart, const float* segEnd)
{
...
int Vec3MinorAxis(const float* dir)
{
...
void Vec2Rotate(const float* in, float angle, float* out)
{
...
void RotationMatrix(const float* dir, float radians, float* matrix[0x3])
{
...
void ProjectPointOntoVectorDir(const float* point, const float* start, const float* dir, float* result_projectedPoint, float* result_projectedDistanceFromStart)
{
...
float PointToLineSegmentDistSq2D(const float* point, const float* start, const float* end)
{
...
float PointToLineSegmentDistSq(const float* point, const float* start, const float* end)
{
...
void QuatNlerp(const float* from, const float* to, float frac, float* result)
{
...
void MatrixToAngleAxis(const float* matrix[0x3], float* axis, float* angle)
{
...
void FinitePerspectiveMatrix(float tanHalfFovX, float tanHalfFovY, float zNear, float zFar, float* mtx[0x4])
{
...
void InfinitePerspectiveMatrix(float tanHalfFovX, float tanHalfFovY, float zNear, float* mtx[0x4])
{
...
void OrthographicMatrix(float width, float height, float depth, float* mtx[0x4])
{
...
void MatrixForViewer(const float* origin, const float* axis[0x3], float* mtx[0x4])
{
...
void MatrixForViewerNoOrigin(const float* axis[0x3], float* mtx[0x4])
{
...
int Intersect2DPlanes(const float* dir1, float dist1, const float* dir2, float dist2, float* result)
{
...
bool CullBoxFromCone(const float* coneOrg, const float* coneDir, float cosHalfFov, const struct Bounds* bounds)
{
...
bool CullBoxFromSphere(const float* sphereOrg, float radius, const struct Bounds* bounds)
{
...
bool PointInsideCone(const float* coneTip, const float* coneDir, float cosHalfFov, float coneHeight, const float* point)
{
...
bool CullBoxFromConicSectionOfSphere(const float* coneOrg, const float* coneDir, float cosHalfFov, float radius, const struct Bounds* bounds)
{
...
void TanHalfAngles(float& tanHalfFovX, float& tanHalfFovY, float fovdeg_x, float viewAspect)
{
