...
float SND_GetTimescaleLerp(const int entchannel)
{
...
float SND_DistSqToNearestListener(const float* origin)
{
...
void SND_SetListener(int localClientNum, int clientNum, const float* origin, const float* axis[0x3], const float* velocity)
{
...
static void DoLengthNotify(int msec, void* lengthNotifyData, enum SndLengthId id, unsigned char index)
{
...
void SND_AddLengthNotify(int playbackId, void* lengthNotifyData, enum SndLengthId id, unsigned char index)
{
...
float SND_GetLerpedSlavePercentage(const struct snd_alias_t* alias)
{
...
float SND_GetChannelBaseVolume(const struct snd_channel_info_t* chaninfo)
{
...
void SND_SetSoundFileChannelInfo(int index, int srcChannelCount, int baserate, enum SndFileLoadingState loadingState)
{
...
static float ReplaceableChannelMetric(struct snd_channel_info_t* chaninfo, bool is3d)
{
...
static int SND_StartAliasVoiced(struct SndStartAliasInfo* startAliasInfo, int* pChannel)
{
...
static void SND_ContinueLoopingSound_Internal(int chanIndex, float lerp, float volumeScale, float pitch, int* pChannel, function * setPlaybackRateFunc)
{
...
static bool SND_ContinueLoopingSound(const struct snd_alias_t* alias0, const struct snd_alias_t* alias1, float lerp, float volumeScale, float pitch, union SndEntHandle sndEnt, const float* org, int* pChannel)
{
...
static bool InRange(const struct snd_alias_t* alias0, const struct snd_alias_t* alias1, float lerp, const float* org)
{
...
static void SND_PlaySecondaryAlias(const struct snd_alias_t* alias, float lerp, float volumeScale, float pitch, union SndEntHandle sndEnt, const float* org, int timeshift, bool treatAsMaster, float timescaleLerp, enum snd_alias_system_t system, const int recursionDepth)
{
...
static int SND_PlaySoundAlias_Internal(const struct snd_alias_t* alias0, const struct snd_alias_t* alias1, float lerp, float volumeScale, float pitch, union SndEntHandle sndEnt, const float* org, int* pChannel, int timeshift, bool treatAsMaster, float timescaleLerp, enum snd_alias_system_t system, const int recursionDepth)
{
...
static void SND_StopSoundsInternal(union SndEntHandle sndEnt, const char* aliasName, const int entChannel)
{
...
void SND_StopSoundChannelOnEnt(union SndEntHandle sndEnt, int channelNum)
{
...
int SND_PlayScaledSoundAlias(const struct snd_alias_t* alias, float volumeScale, float pitch, union SndEntHandle sndEnt, const float* org, int timeshift, enum snd_alias_system_t system)
{
...
int SND_PlaySoundAliasWithPitch(const struct snd_alias_t* alias, float pitch, union SndEntHandle sndEnt, const float* org, int timeshift, enum snd_alias_system_t system)
{
...
bool SND_ValidateSoundAliasBlend(const struct snd_alias_t* alias0, const struct snd_alias_t* alias1, bool throwErrorIfInvalid)
{
...
int SND_PlayBlendedSoundAliasesWithPitch(const struct snd_alias_t* alias0, const struct snd_alias_t* alias1, float lerp, float volumeScale, float pitch, union SndEntHandle sndEnt, const float* org, int timeshift, enum snd_alias_system_t system)
{
...
void SND_SetPauseSettings(unsigned int* pauseSettings)
{
...
void SND_ResetTimeScaleDefaults()
{
...
static void SND_StartBackground(int localClientNum, int track, const struct snd_alias_t* alias, int fadetime, float fraction, float timescaleLerp, enum snd_alias_system_t system)
{
...
void SND_PlayMusicAlias(int localClientNum, const struct snd_alias_t* alias, float timescaleLerp, enum snd_alias_system_t system)
{
...
static bool UpdateChannelSlaveLerp(const int first, const int last, function * isPlayingFunc, float* masterPercentage)
{
...
void SND_SetEqForChannel(int entchannel, int eqIndex, int band, enum SND_EQTYPE type, float gain, float freq, float q)
{
...
int SND_StopChannelAndPlayChainAliasIfTimeExpired(int chanId)
{
