...
static void R_CmdBufUpdateFences(struct GfxCmdRingBuf* cmdBuf)
{
...
void R_CmdBufReserveMemory(struct GfxCmdRingBuf* cmdBuf)
{
...
static void* R_CmdBufAllocCallback(unsigned long Context, unsigned long Flags, unsigned long* pSize, unsigned long Alignment)
{
...
static void R_CmdBufFreeCallback(unsigned long Context)
{
...
static void R_CmdBufQueryCallback(unsigned long Context, unsigned long* pUsed, unsigned long* pRemaining)
{
...
int R_ReleaseDeviceOwnershipForStreaming()
{
...
static void R_ToggleSmpFrameCmd()
{
...
static bool R_HandOffToBackend()
{
...
void R_AddCmdDrawStretchPicRotateST(float x, float y, float w, float h, float centerS, float centerT, float scaleFinalS, float scaleFinalT, float minS, float maxS, float minT, float maxT, float angle, const float* color, struct Material* material)
{
...
static bool R_FogIsWorldLoaded()
{
...
void R_AddCmdRestoreSavedScreen(float viewX, float viewY, float viewWidth, float viewHeight)
{
...
static void R_AddCmdDrawTriangles(struct Material* materialHandle, enum MaterialTechniqueType techType, unsigned char dimensions, short indexCount, const unsigned short* indices, short vertexCount, const float* xyzw[0x4], const float* normal[0x3], const unsigned char* color[0x4], const float* st[0x2])
{
...
void R_AddCmdDrawTriangles2D(struct Material* materialHandle, enum MaterialTechniqueType techType, short indexCount, const unsigned short* indices, short vertexCount, const float* xyzw[0x4], const float* normal[0x3], const unsigned char* color[0x4], const float* st[0x2])
{
...
void R_AddCmdDrawTriangles3D(struct Material* materialHandle, enum MaterialTechniqueType techType, short indexCount, const unsigned short* indices, short vertexCount, const float* xyzw[0x4], const float* normal[0x3], const unsigned char* color[0x4], const float* st[0x2])
{
...
bool R_AnySunHasShadowMap(const struct GfxBackEndData* data)
{
