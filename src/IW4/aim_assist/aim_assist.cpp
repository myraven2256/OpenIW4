...
static float AimAssist_LerpDvars(const struct dvar_t* from, const struct dvar_t* to, float frac)
{
...
static float AimAssist_GetClipWidthFromVirtualWidth(int localClientNum, const struct ScreenPlacement* scrPlace, float virtualWidth)
{
...
static float AimAssist_GetClipHeightFromVirtualHeight(int localClientNum, const struct ScreenPlacement* scrPlace, float virtualHeight)
{
...
static void AimAssist_UpdateTweakables(int localClientNum)
{
...
void AimAssist_Setup(int localClientNum, const struct playerState_s* ps)
{
...
void AimAssist_BackupPlayerState(int localClientNum, const struct playerState_s* ps)
{
...
static void AimAssist_CreateScreenMatrix(int localClientNum, struct AimAssistGlobals* aaGlob, float tanHalfFovX, float tanHalfFovY)
{
...
static bool AimAssist_ConvertToClipBounds(const struct AimAssistGlobals* aaGlob, const struct Bounds* bounds, const float* mtx[0x3], struct ExtentBounds* clipBounds)
{
...
static float AimAssist_GetAimAssistRange(int weapIndex, float adsLerp)
{
...
static float AimAssist_GetAutoAimRange(int weapIndex)
{
...
static int AimAssist_GetTagPos(int localClientNum, const struct centity_s* cent, unsigned int tagName, float* pos)
{
...
void AimAssist_UpdateScreenTargets(int localClientNum, const float* eyeOrg, const float* viewOrg, const float* viewAngles, float tanHalfFovX, float tanHalfFovY)
{
...
static bool AimAssist_IsPlayerStunned(const struct AimAssistPlayerState* ps)
{
...
static bool AimAssist_IsPlayerUsingOffhand(const struct AimAssistPlayerState* ps)
{
...
static int AimAssist_IsSlowdownActive(const struct AimAssistPlayerState* ps)
{
...
static bool AimAssist_UpdateAutoMeleeTarget(struct AimAssistGlobals* aaGlob, const struct AimInput* input)
{
...
static void AimAssist_DrawCenterBox(const struct AimAssistGlobals* aaGlob, float verticalOffset, float clipHalfWidth, float clipHalfHeight, const float* color)
{
